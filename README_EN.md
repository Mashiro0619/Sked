...
In-app updates default to stable releases. Enable **Settings → About → Receive prerelease updates** to include Alpha, Beta, and RC releases. Versions follow SemVer precedence, including an RC upgrading to its matching final release. See [update versions and channels](docs/updates.md) for release naming and custom feed formats.

Windows notifications use system toasts. Build portable binaries with `flutter build windows --release`. Use `pwsh -File tool/build_msix.ps1 -Store` for a Microsoft Store submission package; the Store signs it, so no local signing certificate is required. See [Microsoft Store publishing](docs/microsoft-store.md) for product identity, version mapping and certification checks. For non-Store distribution, `-Unsigned` creates a validation package, while signed packages require `SKED_MSIX_CERTIFICATE_PATH`, `SKED_MSIX_CERTIFICATE_PASSWORD`, and `SKED_MSIX_PUBLISHER` before running the script without switches. Every script mode rebuilds the matching update channel. MSIX package identity is required to reliably cancel notifications already shown in Action Center; unpackaged builds can show and schedule notifications, but history lookup and cancellation are not guaranteed.
