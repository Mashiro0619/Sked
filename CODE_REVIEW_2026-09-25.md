...
对 `fec74b5` 和 `88d187a` 的二次审查发现：上一节第 5、6 项“已修复”的结论缺少两个边界场景，不能仅凭原有测试通过就认定完全闭环。本节保留原始审查和首轮验证记录，并补充遗漏的行为与修复证据。

### 补修内容

| 对应问题 | 二次审查发现 | 本轮处理 |
| --- | --- | --- |
| 第 6 项：磁盘事务 | 回滚把 `.tmp` 改写为旧快照后，异常分支仍按新快照校验归属，隔离失败被吞掉；下次加载可能把旧快照提升到期间出现的较新主文件之上。 | 跟踪当前拥有的临时快照；进入回滚写入阶段即切换为旧主文件快照。回滚校验失败时将该副本正确隔离为恢复产物，保留较新主文件与经过验证的旧备份。仍无法隔离的故障继续报告结果未知，不承诺已经撤回。 |
| 第 5 项：周次恢复 | 20 周、选中第 12 周时缩短到 8 周，失败回滚恢复了总周数，却留下第 8 周；删除非活动课表失败也会无关地跳周。 | 配置修改保存失败后恢复修改前的选择，再按回滚后的周数校正；用选择代次防止覆盖等待期间发生的新导航。删除非活动课表不再重置周次，活动身份变化仍按原规则定位。 |
| 存量通知锁问题 | 协调器持有运行时锁，而动作轮询先占用服务对账队列再等待该锁，双方互等；父提交对照也可复现，非 `88d187a` 新引入。 | 服务统一先取得运行时锁，再预约本地队列；重入请求先预约后等待，保持 FIFO。等待锁的动作轮询在取得锁后读取已提交快照，不能覆盖刚提交的新计划。清除代次取消等待任务，同时保留首次投影前及清除中的轮询门禁。 |

### 新增正式回归

- `test/review_regression_test.dart`：回滚临时文件隔离、恢复产物可读性、重新加载及重建存储实例后保留较新主文件；缩短周数的失败恢复与成功校正、等待期间的新导航（包含先离开再回到第 8 周）、扩展周数失败后的越界校正、活动／非活动课表删除失败。
- `test/services/agenda_coordinator_test.dart`：恢复对账与动作轮询／直接服务请求并发不互等；轮询不覆盖新提交的通知设置；清除取消等待请求；首次投影前及清除中的轮询不重建计划；同一可重入锁区域内的多次对账保持 FIFO 且不并行排程。
- 先加入正确行为断言并确认跑红，再实施修复；没有把“错误确实发生”的断言当作通过标准。

### 本轮验收结果

- **完整 Flutter 测试**：`flutter test --no-pub --coverage`，3411 项通过、1 项按命名时区入口的既有规则跳过；比首轮新增 16 项正式回归。
- **专项回归**：存储、Provider、通知服务及协调器最终 169 项全部通过；原二次审查的 3 条隔离复现按正确行为断言也全部通过。课表删除用例改用固定的第二课表 ID，避免内存测试在同一毫秒添加两个课表时出现身份碰撞。
- **静态与同步检查**：`flutter analyze --no-pub` 无问题；改动 Dart 文件格式检查、`git diff --check` 通过；重新执行 `flutter gen-l10n` 后多语言生成文件无差异。
- **覆盖率**：以 `88d187a` 为本轮基线，整体 `39377/43078 = 91.4086%`，改动行 `44/44 = 100%`，通过现有整体门槛及改动行 90% 门槛；没有降低门槛或新增覆盖率排除。
- **Windows 构建**：隔离烟测入口与正式 `lib/main.dart` 的 Debug 构建均通过，验证后已恢复正式 Debug 产物；原有第三方 WebView CMake 告警不属于本轮修复。
- **Windows 原生烟测**：使用纯内存课表、密钥和恢复日志运行独立 Debug 入口。实际日期导航后列宽为 160；注入总周数保存失败后，恢复为 20 周且仍选第 12 周；注入密钥失败后发送原生 `WM_CLOSE`，关闭回调返回允许，测试进程退出码为 0。此项仍不等同于完整鼠标交互或真实磁盘故障实验。
- **验证限制**：本轮没有 Android 真机通知投递／操作验证，没有 WACK、微软商店认证或真实磁盘满／杀毒软件占用实验；没有把故障注入或 Windows 烟测视为这些平台实验已经完成。

本轮继续保持 AppData／备份 JSON schema、通知 payload、应用版本与依赖声明不变；不新建或上传 Store MSIX 包、不 push。修复、正式回归及本节报告一起做本地 commit，原有未跟踪 `.flutter_tool_state` 不纳入提交。
