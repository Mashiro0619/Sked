...
更新：2026-09-15。最新修订分别处理紧凑触控与桌面／平板展示，承接工作区安全基础，未提交、未推送。本轮结果单独记录于 `WORKBENCH_UI_REVISION.md`；以下融合方案的历史验收不能替代修订后的验证。
...
大屏预算随字体放大。空间不足先折叠／隐藏资源，再覆盖详情，最后覆盖 AI；不覆写用户的打开状态和宽度偏好。两个覆盖任务只显示最近操作的一项，另一个保持会话但不取得焦点。手机及紧凑触控窗口的课程／日程详情、编辑和关联清单独立使用底部模态任务，不套用大屏覆盖面板；新任务继承已打开的移动模态栈，旋转不搬移现有草稿。

- 课表保留完整画布、日期、空闲节次与间隔；日／周日历默认不重复当天清单。月历横向预算充足时旁置安排，竖屏有足够高度时月历＋下方安排填满空间；窄窗口使用单滚动面并在选中月底后返回安排。列表采用列表—详情。
- 日程紧凑触控周视图默认整周铺满（按周末设置显示 5 / 7 列），可在通用显示设置关闭「周视图适应屏幕」改为宽列横向浏览；超过 7 天的自定义范围仍横向浏览。桌面列宽策略及课表原有两种模式不变。表头、全天区、时间网格使用同一列坐标，横滚与切周互斥；不删日期、不缩小全局字号、不改变真实时长和日期导航规则。
- 手机及紧凑触控窗口的首页操作栏与内容使用同一底色，大屏继续区分框架层与内容层。日程手机栏不显示左右日期按钮，分类、日期、视图和操作入口统一放在一行；空间不足时先省略同年日期的年份，为完整分类名称让位；仍放不下时才截断名称或继续缩略日期。空间足够恢复年份，跨年范围保留年份，完整日期始终保留在语义／提示中。保留导航排序、隐藏偏好及触控目标，翻周由日历滑动或日期弹层完成。
- 手机日期表头按实际文字测量高度，普通字号为 48 dp，上下内边距 4 dp；系统安全区保留且不重复占位。月份／年份层使用四列优先的紧凑网格，大字号自动减列，触摸不显示键盘焦点方框。桌面与宽平板的高度和选择器样式保持不变。
- 手机普通周日期弹层保持点按跳周，显式拖选转为自定义范围；自定义入口同时支持两点和拖选。移动月历合并月份／关闭栏，仅显示覆盖当月的 4–6 行，以连续浅色选区和圆形端点反馈，不常驻键盘焦点框。范围预览不保存，合法松手仅保存一次。
- 手机选择器按任务分型：跳周为周标题附近的紧凑锚定浮层（透明模态阻挡层），业务单日与所有时间选择为居中卡片（24 dp 圆角、24% 黑色遮罩）。锚点优先下方、其次上方，不可用则居中；边界避让安全区和键盘，浮层不涂满导航栏区域。主页切周、月份导航、范围拖选仍使用原来的底部弹层，外层课程／日程详情和编辑路由不变，Windows 与宽平板保留原样。
- 手机时间默认三行循环滚轮，共用一条选中带；通过键盘／滚轮按钮切换手动输入，不再同时堆放输入框和两列滚轮。两种模式共享草稿，滚轮运动或输入非法时禁止切换／确定，取消始终可用；小时、分钟独立回绕，不进位、不改上午／下午。单日卡片只保留月份导航与底部取消／确定，次要操作靠左，字号放大时换行。320 dp 七列日期无法保证每格 48 dp 宽，按实测字形先减横向留白，保持 48 dp 行高，不以缩字冒充触控预算达标。
- 连续表面和细分隔取代整栏浮动大卡片；课表／日历使用剩余空间，普通表单限制阅读宽度。
- `WorkspacePaneController` 区分大屏 Navigator 任务与手机底部路由，统一登记对象、关闭目标及会话；手机模态不让后台出现空白检查器。`WorkspaceTaskScope` 仅用于填满大屏面板，移动弹窗按内容／已有编辑高度展示，输入法高度只扣一次。
...
| 日程周视图适应屏幕 | 日程 → 更多 → 显示与交互 | 默认开启；仅影响紧凑触控布局，独立于课表同名偏好；重启及完整备份恢复保留 |
