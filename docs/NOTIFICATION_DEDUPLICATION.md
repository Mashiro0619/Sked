...
## 2026-09-16：运行时锁顺序与过期记录清理

后续手机视图切换排查中，通过回归复现了两类独立的通知存储死锁；不是从截图推断投递或死锁已发生：

1. **跨锁顺序反转。** 外部写入先占用实例队列再等待全局锁；当前持有全局锁的对账又需要同一实例队列。现在统一先取得跨引擎运行时锁，再预留实例队列；可重入锁域中的并发写仍由实例队列串行化。
2. **清理等待自身。** 删除稍后记录、已处理记录或待消费动作时，公共读取发现过期／旧格式记录并把清理排到当前写入之后，然后当前写入又等待清理。现在这些删除操作使用不另行排队的代际读取，在本次持锁写入中同时过滤过期数据、迁移旧已处理时间戳并删除目标。

没有新增生产超时、跳过持久化占位或关闭通知来掩盖等待。登记证据、单次补发、稍后动作凭据、代际守卫及 Windows 全副本清理契约不变。纯日历日期／范围导航不再借用通知互斥锁；真正的工作区启停仍保留通知隔离。

新增 `notification_runtime_lock_order_test` 共七项，验证外部写入与持锁者交错、同锁域并发写不丢失、写失败后的队列与锁释放，以及过期稍后／已处理／动作、旧格式已处理记录删除。四项清理回归在修复前均能稳定触发超时，修复后完成并保留其他有效记录；测试用的超时仅用于识别死锁，不进入生产实现。界面、视觉和全量验证结果见 `WORKBENCH_UI_REVISION.md` 的 2026-09-16 记录。
