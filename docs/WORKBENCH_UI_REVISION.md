...
## 2026-09-18：轻量 MD3E 总览、底栏与月历

本轮替代未提交的“底栏与系统手势区分色”实验。此前只证明图文容器在 64 dp 内居中，并没有证明真实字形的视觉平衡；也不应为了设置首页简短而把常用操作全部藏进分类页。

### 实现

- 底栏保留官方 NavigationBar、64 dp 内容区和一个系统安全区，使用连续背景；去掉常驻选中胶囊，选中／未选中图标均向下 4 dp。选中依靠实心图标、主题色族和适度字重，仍保留焦点、按压、键盘和无障碍行为。小型前景强调色在对比不足时使用现有 MD3 色族的可读色阶，不改变用户保存的主题色。
- 设置使用分组总览：外观与语言、学生课表、通用日程、通知、工作区功能、数据与隐私、关于。每组一块低对比底色与 24 dp 圆角，统一图标、文字和尾部控件位置，不逐行套卡片；宽屏左侧定位分组，右侧独立标题栏避让原生窗口按钮。初版将过多细项直接展开，已按本节后续反馈修订收回二级页。
- 总览与详情沿用原有主题、通知、工作区启停、备份和数据清除保存流程。主题配置目标不会切换活动工作区；保存／权限请求未结束时，嵌套根页的返回保护也不能被顶部返回或系统返回绕过。周列、网格、农历、列宽、默认提醒提前量等细项仅在各自详情页配置；一级仍有清晰入口。
- 所有主页及紧凑“更多”菜单移除独立显示偏好入口，普通设置、日历操作、资源管理和添加入口仍保留。课表／日程显示详情页在宽屏中不再与左侧索引同时显示两个返回按钮。
- 课程专属提醒移入“更多”，取消通知设置跳转按钮。折叠摘要保留默认／关闭／自定义状态；非法自定义分钟数会展开并聚焦错误，不静默降级为默认提醒。
- 新增可空的 `customDayMinWidth` 与 `updateGeneralCustomDayMinWidth(int?)`：自动模式兼容旧布局；手动初值 96 dp，64–320 dp、8 dp 步长。只影响实际 Custom 范围，包括七天自定义；等宽铺满或横向滚动。标题、全天和时间网格共享布局与命中指标，变化时保留日期／时间锚点。完整备份保留该值，内容导入不覆盖；AppData 和日程数据版本不变。
- 紧凑月视图改为月历在上、当日日程在下的一条 sliver 滚动流。普通日期行基准 48／56 dp（无／有农历），移除上方 188 dp 日程占位，空日程只显示一行。真实字体检查还发现 320 dp／2 倍字下两位数日期会被内部选中背景裁掉；已分离背景与字形布局，允许大字号农历换行，并为实际字形增加断言。宽屏仍保留事件画布与侧边日程。
...
| 全量软件回归 | **2929 项通过，1 项既有 DST 条件跳过**；没有新增跳过项 |
| 覆盖率门禁 | **PASS**：整体 **38645/42417（91.1073%）**，改动行 **822/839（97.9738%）**；源文件清单通过，未调整门槛或排除规则 |
| 实际字体视觉矩阵 | **31 组配置、279 张最终截图**通过，包括两位数日期实际字形存在／不跨列断言；提交基线另有 186 张图 |
| 真实 Win32 缩放 | **6 个场景通过**：总览宽 360／660／1000／1440 dp，详情宽 360／1000 dp；120 DPI。Flutter 渲染尺寸与真实客户区一致，所有最大化区域返回 HTMAXBUTTON（9） |
| 边界 | Android 无 ADB 设备；MSIX 安装包及真实前台鼠标／贴靠仍待设备复核。版本和依赖未变，未提交／推送 |

前后对照位于 `.scratch/md3e-comparison/`：`settings-and-month.png`、`bottom-navigation.png`、`controls-and-accessibility.png`。

### 验证方式与证据

- 回归覆盖：总览直接操作、保存失败、通知先保存后申请权限、权限失败后的重试、嵌套返回保护、主题目标、旧设置入口移除、折叠提醒校验与保留，以及原有通知去重／视图切换／编辑器草稿保护。
- 数据与日历覆盖：旧 schema 缺省、空值／手动宽度、严格磁盘校验、完整备份往返、内容导入、失败回滚、禁用工作区；1／5／7／10／14 天、普通周不受影响、列头与点击坐标一致、横纵滚动锚点。
- 原始真实字体基线：`.scratch/md3e-before/`，从 HEAD `73cb9c8` 的独立源快照构建，不包含被放弃的未提交分色层；该层的历史截图仍在原 `android-navigation-surface-after` 目录。
- 最终视觉矩阵：`.scratch/md3e-after-final/`；每个手机宽度 320／360／393／412 dp 覆盖字号 1／1.3／2 与明暗主题，另含英文、平板及 Windows。包含底栏、设置总览、自定义列宽、课程编辑和空／非空月历，附 JSON 清单。图片使用实际 Windows Flutter 渲染与系统字体；Android 尺寸、安全区是模拟值。
- 原生 Windows 检查另用 Win32 改变真实 HWND 尺寸，布局完成后通过只读 `inspect` 检查 HTMAXBUTTON 命中区域，记录在同目录 `native-window-evidence.json`；不是仅修改测试 view 的 physicalSize。宿主未抢占前台，最初桌面截图被遮挡安全检查拒绝，因此最终只捕获本应用 Flutter 表面，不关闭遮挡检查、不记录其它桌面窗口；不将它表述为已验证真实鼠标悬停贴靠或 MSIX。
- 最终分析、全量测试与覆盖率门禁日志分别保存在 `.scratch/md3e-final-analyze.log`、`.scratch/md3e-final-tests.log`、`.scratch/md3e-final-gate.log`；视觉／原生窗口日志为 `.scratch/md3e-final-native.log`。门禁和排除规则不降低。
...
### 2026-09-18 反馈修订：侧栏动效与设置层级

本次没有回退已经修好的手机底栏、紧凑月历、自定义列宽能力或课程提醒折叠，也没有改通知调度、依赖及版本。

- **集成资源侧栏不再替换控件树。** 原先展开用 ListTile、折叠换成更矮且图标更小的 IconButton，累计造成工作区图标和底部设置上下移动。现在两态复用同一个 ListTile，统一行高、图标尺寸和起始槽位；标题、资源入口及底部设置使用同一图标轴线。宽度按现有动效策略过渡，文字和资源内容按实际宽度裁切淡入淡出，日历不会在过渡中被挤窄重排。保留列表滚动与设置焦点；减少动态效果直接到位，窗口缩小时始终给主画布及新停靠任务留足预算。
- **语言恢复独立搜索页面。** 一级只显示当前语言和入口；手机成功保存后返回，宽屏留在页面，窗口缩放保留搜索，保存未完成禁止返回，失败可以重试。
- **节次时间集选择恢复弹窗。** 设置和学生资源菜单复用现有选择器，新增／编辑仍能进入节次编辑；选择绑定打开时的课表，成功保存后才关闭，重复确认不重复保存，失败留在弹窗重试。没有活动课表也能管理节次模板。历史直接管理路由保留兼容，不再作为主要选择入口。
- **一级不再铺满细项。** 主题常用设置和通知总开关保留直接操作；周列／网格、日程视图／周末／农历／列宽、默认提醒提前量、工作区启停、隐私与清除数据、关于细节各回对应二级页。导入导出、备份、版本检查仍有清晰入口；“显示与交互”继续属于设置，不回到主页或更多菜单。
- **回归按真实交互检查。** 新增中间帧与往返坐标、RTL、字号 1／1.3／2、焦点、滚动位置、减少动态效果、动画中途缩窗、语言保存失败和弹窗重复确认测试。视觉脚本补充两工作区侧栏展开／折叠／中间帧、独立语言页和节次时间集弹窗，并通过二级显示页操作列宽。
...
| --- | --- |
| 静态分析 | 通过，无问题 |
| 全量测试 | **2947 项通过，1 项既有 DST 条件跳过**。首次高并发运行的 pre-commit 工具测试触发原有 45 秒超时；定向复测和并发度 2 的完整重跑均通过，未修改测试超时、断言或覆盖率门槛 |
| 覆盖率 | **PASS**：整体 **38703/42479（91.1109%）**；改动行 **992/1022（97.0646%）**；源文件清单通过 |
| 实际字体视觉矩阵 | **31 组配置、353 张截图**，包含 12 张两工作区侧栏状态／过渡帧以及语言独立页、节次时间集弹窗；Android 尺寸及安全区仍是模拟 |
| 真实 Win32 缩放 | **6 个场景通过**；客户区与 Flutter 宽度一致，最大化区命中 HTMAXBUTTON（9）。测试窗口未抢占前台，不代表已完成人工鼠标悬停、Snap 或 MSIX 验收 |

记录：

- 软件回归：`.scratch/sidebar-settings-full-final.log`、`.scratch/sidebar-settings-analyze-final.log`、`.scratch/sidebar-settings-coverage.md`。
- 视觉与原生：`.scratch/sidebar-settings-visual/`，含截图清单和 `native-window-evidence.json`；测试日志为 `.scratch/sidebar-settings-visual.log`。
- 侧栏过渡对照：`.scratch/sidebar-settings-comparison/sidebar-transition.png`。
- 设置层级与选择器对照：`.scratch/sidebar-settings-comparison/settings-hierarchy.png`。
- Windows 构建仍有第三方插件既有 CMake 警告，本轮未修改依赖。版本保持 `2.3.0+14`，无自动提交或推送。


### 2026-09-18 局部反馈：底栏按压、面板关闭与分类入口

- 仅底部工作区 NavigationBar 使用 NoSplash，并去掉按压／悬停带来的临时胶囊底色；选中图标和文字、键盘焦点、键盘激活及无障碍语义不变。其它按钮不继承这项局部设置。
- 应用内详情面板的关闭按钮四周增加 8 dp 外边距，停靠和覆盖布局一致；不修改原生窗口关闭按钮，也不绕过草稿退出守卫。
- 删除“分类”标题旁重复的更多按钮。原菜单的分类管理由左侧分类图标直接进入，新增分类仍有加号，导入／导出继续位于设置；不是删除这些能力。
- 回归新增实际底栏像素比较：按下已选／未选工作区及鼠标悬停不绘制波纹或底色，键盘焦点仍产生可见变化并能 Enter 切换。面板边距覆盖 LTR／RTL、1／2 倍字和 330–2400 dp 宽度，同时检查关闭只退出面板、分类管理与设置导入导出可达。

#### 2026-09-19 验证结果

- 静态分析通过；全量 **2955 项测试通过，1 项既有 DST 条件跳过**。
- 覆盖率门禁 **PASS**：整体 **38684/42473（91.0790%）**，改动行 **1004/1034（97.0986%）**；门槛和排除规则未改。
